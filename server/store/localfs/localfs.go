...
func (c *store) Push(ctx context.Context, ref types.Object, contents io.Reader) (types.Object, error) {
...
loop:
	for {
		...
		select {
		case <-ctx.Done():
			return nil, status.Error(codes.Canceled, "context expired") //nolint:wrapcheck
		default:
			...
			if err != nil {
				return nil, status.Errorf(codes.Internal, "failed to read contents: %v", err)
			}

			// Write contents to the file (only the bytes that were read)
			_, err = contentsFile.Write(chunk[:n])
			if err != nil {
				return nil, status.Errorf(codes.Internal, "failed to write contents: %v", err)
			}
	...
	// cid := digest.NewDigestFromBytes(digest.SHA256, hasher.Sum(nil)).String()

	// Store contents
	if err := c.dataFs.Rename(contentsFile.Name(), ref.CID()); err != nil {
		if errors.Is(err, afero.ErrFileExists) {
			return nil, status.Errorf(codes.FailedPrecondition, "file already exists: %v", ref.CID())
		}
	...
	// Update metadata
	metadataRef := &storetypes.Object{
		CIDVal:         ref.CID(),
		SizeVal:        size,
		TypeVal:        ref.Type(),
		AnnotationsVal: ref.Annotations(),
	}
	...
	// Store metadata
	err = afero.WriteReader(c.metaFs, ref.CID(), bytes.NewReader(metadataRaw))
	if err != nil {
		if errors.Is(err, afero.ErrFileExists) {
			return nil, status.Errorf(codes.FailedPrecondition, "metadata already exists: %s", ref.CID())
		}
...
func (c *store) Lookup(_ context.Context, ref types.ObjectRef) (types.Object, error) {
	logger.Debug("Looking up object in LocalFS store", "ref", ref)

	// Read metadata
	metadataRaw, err := afero.ReadFile(c.metaFs, ref.CID())
	if err != nil {
	...
	// Process metadata
	var metadata storetypes.Object
...
func (c *store) Pull(_ context.Context, ref types.ObjectRef) (io.ReadCloser, error) {
	logger.Debug("Pulling object from LocalFS store", "ref", ref)

	file, err := c.dataFs.Open(ref.CID())
	if err != nil {
		if errors.Is(err, afero.ErrFileNotFound) {
			return nil, status.Errorf(codes.NotFound, "object not found: %s", ref.CID())
		}
...
func (c *store) Delete(_ context.Context, ref types.ObjectRef) error {
	...
	err := c.dataFs.Remove(ref.CID())
	if err != nil {
		if errors.Is(err, afero.ErrFileNotFound) {
			return status.Errorf(codes.NotFound, "object not found: %s", ref.CID())
		}

		return status.Errorf(codes.Internal, "failed to remove contents: %v", err)
	}

	err = c.metaFs.Remove(ref.CID())
	if err != nil {
		if errors.Is(err, afero.ErrFileNotFound) {
			return status.Errorf(codes.NotFound, "metadata not found: %s", ref.CID())
		}
